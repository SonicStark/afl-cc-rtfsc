...
static void defs_selective_instr(aflcc_state_t *aflcc) {
...
/* As documented in instrumentation/README.persistent_mode.md, deferred
    ...
    and afl-clang. */
static void defs_persistent_mode(aflcc_state_t *aflcc) {
...
void add_macro(aflcc_state_t *aflcc) {

  defs_common(aflcc);
  defs_selective_instr(aflcc);
  defs_persistent_mode(aflcc);

}

/* Control  _FORTIFY_SOURCE */
void set_fortification(aflcc_state_t *aflcc, u8 action) {
